...
class AutoDataCollector(Node):
    ...
    def sampling_process(self):
        ...
        while self.twist_msg == None: time.sleep(0.1)
    ...
    def communicator(self):
        ...
        while iterations:
            ...
            for i in instructions:
                ...
                if ins == 'MOVE':
                    self.move_x_meters_feedback(float(mag))
                
                if ins == 'RGHT':
                    self.rotate_x_radians_right(float(mag) * math.py / 180)
                    
                if ins == 'LEFT':
                    self.rotate_x_radians_left(float(mag) * math.py / 180)
            ...
            with open(os.path.join("datalogs_auto",fname),'wb') as f:
                ...
                q = time.time() - t_start
                time_total += q
                print(f"Time taken: {round(q,2)}; Avg. time per prompt: {time_total / (args.samples - iterations)}")
            ...
            x, y, _, __ = self.odometry_msg_data
            ...
            if abs(x) > 3 or abs(y) > 3:
                ...
                print(f"Reset location to origin. {self.odometry_msg_data[0:2]}")
