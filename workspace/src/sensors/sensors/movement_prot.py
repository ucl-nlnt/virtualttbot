...
This file is mostly a prototyping file. Here, we can develop stuff without ruining the main files.
...
class AutoDataCollector(Node):
    ...
    def __init__(self, comm_on = True, sampler_on = True, debug = False):
        ...
        """ # not maintained in dev versions
        if comm_on:
    ...
    def debug(self):
        ...
        while self.odometry_msg == None: time.sleep(0.1); print('Waiting for data...')# wait for data to be ready

        # add commands here to debug
        while True:
            print(round(rad_to_deg(quaternion_to_yaw(*self.odometry_msg_data)),3))
            time.sleep(1)
    ...
    def sampling_process(self): # not used during debug mostly;
    ...
    def communicator(self): # not used during debug mostly as well
