        ...
        while not self.killswitch:
            ...
            for i in inference:
                ...
                There are three possible cases:
                    1.) rotation
                    2.) move forward
                    3.) rotation then move forward
                """

                # difference in coordinates
                if not math.isclose(x2,x1,abs_tol=0.1) or not math.isclose(y2,y1,abs_tol=0.1):
                    
                    # rotate towards direction
                    
                    dtheta = math.atan2(y2-y1,x2-x1)
                    
                    print(f'Facing direction {dtheta}.')
                    self.movement_rotate_until(dtheta,tolerance=0.01,rotation_s=0.1)
                    print('Rotation complete.')

                    # move towards direction

                    print('Moving to new coordinate.')
                    self.movement_forward_until_distance(math.sqrt((x2-x1)**2 + (y2-y1)**2),tolerance=0.01,linear_s=0.22)
                    print(f'Movement Complete.')

                    
                # convert to same polarity for correct comparison.
                ...
                # rotate to face new coordinates

                if not math.isclose(phi1,copy_phi2,abs_tol=0.0872665):
                    print('Rotating to correct angular displacement...')
                    self.movement_rotate_until(phi2,tolerance=0.01,rotation_s=0.1)
                    print('Rotation complete.')

                print('Current Coordinate:',[round(i,3) for i in self.position_odom])
